...
---------------------------------------
--
readL1prog :: FilePath -> IO ()
-- Acciones Haskell para:
--      leer un archivo de texto que contiene un programa L1
--      aplicar parsing
...
readL1prog fileName
    = do
        ...
        textProgL1  <- hGetContents handle
        -- aplicar parsing
        let progL1 = (parse tokenProgL1 textProgL1)
        -- regresa proL1:
        print progL1
        hClose handle -- cerrar el archivo.
--

readAndShowL1prog :: FilePath -> IO ()
-- Acciones Haskell para:
--      leer un archivo de texto que contiene un programa L1
--      aplicar parsing
--      mostrar el resultado del parsing
readAndShowL1prog fileName
    = do
        -- abrir (declarar) el archivo:
        handle      <- openFile fileName ReadMode
        -- leer el contenido del archivo:
...
parseProgStms = parse tokenProgL1 progStmsStr

checkParsingProgStms :: Bool
checkParsingProgStms =
    progStms == fst (head (parse tokenProgL1 progStmsStr))
------------------------------------------
--
